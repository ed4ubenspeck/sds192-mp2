...
#### In order to produce a meaningful graphic, it was necessary to organize the relevant data into a format compatible with our vision^[[GitHub Repository](https://github.com/ed4ubenspeck/sds192-mp2.git)]. We first performed a join for the dataframes ``candidates`` and ``contributions`` by the variable `cand_id` to yield a dataframe with contributions organized by candidate. Relevant variables were selected, rows with missing data omitted, and this dataframe informed the creation of three functions. The first and second of these functions perform filter for candidates from a specific state, calculate our values for the x- and y-axes in the final plot, and perform a second join with ``house_elections`` to determine if a particular candidate won or lost their general election. The third function creates a scatterplot for the state of interest, enhanced by a stat_smooth layer to show linear models and confidence intervals for the position of winning and losing candidates within the distribution. 
